...
use elrond_wasm_debug::{managed_token_id, rust_biguint, testing_framework::*, DebugApi};
use entity::esdt::*;
use entity::*;
    ...
    ObjBuilder: 'static + Copy + Fn(DebugApi) -> entity::ContractObj<DebugApi>,
{
    ...
    pub owner_address: ManagedAddress,
    pub contract: ContractObjWrapper<entity::ContractObj<DebugApi>, ObjBuilder>,
}

fn setup_entity<ObjBuilder>(builder: ObjBuilder) -> EntitySetup<ObjBuilder>
where
    ObjBuilder: 'static + Copy + Fn(DebugApi) -> entity::ContractObj<DebugApi>,
{
