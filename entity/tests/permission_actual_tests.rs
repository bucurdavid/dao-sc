...
fn it_matches_a_permission_based_on_destination_only() {
    ...
    setup.blockchain.execute_tx(&setup.owner_address, &setup.contract, &rust_biguint!(0), |sc| {
        ...
        sc.create_permission(managed_buffer!(b"addressOnlyPerm"), managed_address!(&action_receiver), managed_buffer!(b""), ManagedVec::new());
...
fn it_matches_a_permission_based_on_destination_and_endpoint() {
    ...
    setup.blockchain.execute_tx(&setup.owner_address, &setup.contract, &rust_biguint!(0), |sc| {
        sc.create_role(managed_buffer!(b"builder"));
        sc.assign_role(managed_address!(&proposer_address), managed_buffer!(b"builder"));
        sc.create_permission(managed_buffer!(b"addressAndEndpoint"), managed_address!(&action_receiver), managed_buffer!(b"myendpoint"), ManagedVec::new());
...
fn it_matches_a_permission_based_on_destination_and_endpoint_and_one_argument() {
    ...
    setup.blockchain.execute_tx(&setup.owner_address, &setup.contract, &rust_biguint!(0), |sc| {
        ...
        sc.create_permission(managed_buffer!(b"addressAndEndpoint"), managed_address!(&action_receiver), managed_buffer!(b"myendpoint"), ManagedVec::from(vec![
            managed_buffer!(b"testarg")
        ]));
...
#[test]
fn it_fails_to_match_a_permission_if_a_required_argument_is_missing() {
    let mut setup = EntitySetup::new(entity::contract_obj);
    let proposer_address = setup.user_address.clone();
    let executor_address = setup.blockchain.create_user_account(&rust_biguint!(5));
    let action_receiver = setup.blockchain.create_user_account(&rust_biguint!(0));
    ...
    setup.blockchain.execute_tx(&setup.owner_address, &setup.contract, &rust_biguint!(0), |sc| {
        ...
        sc.create_permission(managed_buffer!(b"perm"), managed_address!(&action_receiver), managed_buffer!(b"myendpoint"), ManagedVec::from(vec![
            managed_buffer!(b"testarg1"), managed_buffer!(b"testarg2")
        ]));
